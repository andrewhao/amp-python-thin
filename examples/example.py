...
python example.py --amp_agents 'http://amp2-dev-clientsim-0,http://amp2-dev-clientsim-1' --key '1622f1831b45819b'
"""
import argparse
import logging
from si import amp
...
def example_code(amp_agents, key):
    ...
    # If there is a problem with the key, or any of the amp_agents,
    # it doesn't make sense to create an amp object,
    ...
    # Typical usage is to send all the pre decision observes and the decide event via a single
    ...
    result = first_session.multi_decide_with_context(context_name="AmpSession",
    ...
    # After the decision is made, send a few more observes if possible:
    # this helps improve future decisions.
...
def get_command_line_options():
    ...
    """
    description = 'This is a program that uses the SI python thin client'
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('--amp_agents',
                        help='amp_agents addresses e.g. localhost:8100,amp2-dev-clientsim-0',
                        required=True)
